...
async def fetch_mlb_news() -> list[dict]:
    """Try multiple RSS sources for MLB news."""
    sources = [
        "https://api.rss2json.com/v1/api.json?rss_url=https://www.mlb.com/feeds/news/rss.xml",
        "https://api.rss2json.com/v1/api.json?rss_url=https://feeds.feedburner.com/sportingnews/baseball",
    ]
    for url in sources:
        try:
            async with httpx.AsyncClient(timeout=15) as client:
            ...
            logger.error(f"News source {url} error: {e}")
    return []
...
class FantraxClient:
    ...
    def __init__(self):
        # Use a cookie jar so login cookies persist across requests
        self._cookies: dict = {}
    ...
    def _store_cookies(self, response: httpx.Response):
        """Extract and store cookies from response."""
        for key, value in response.cookies.items():
            self._cookies[key] = value
        logger.info(f"Stored cookies: {list(self._cookies.keys())}")

    async def login(self) -> bool:
        """Try multiple Fantrax login endpoints."""
        endpoints = [
            {
        ...
        for ep in endpoints:
            try:
                ...
                logger.info(f"Login attempt {ep['url']}: {resp.status_code}")
    ...
    async def _get(self, url: str, params: dict = None) -> Optional[httpx.Response]:
        try:
            async with httpx.AsyncClient(timeout=30, follow_redirects=True) as client:
                resp = await client.get(url, params=params, headers=self._make_headers())
            self._store_cookies(resp)
            logger.info(f"GET {url}: {resp.status_code} | body[:200]: {resp.text[:200]}")
            return resp
        except Exception as e:
            logger.error(f"GET {url} error: {e}")
            return None

    async def _post(self, url: str, body: dict) -> Optional[httpx.Response]:
        try:
            async with httpx.AsyncClient(timeout=30, follow_redirects=True) as client:
                resp = await client.post(url, json=body, headers=self._make_headers())
            self._store_cookies(resp)
            logger.info(f"POST {url}: {resp.status_code} | body[:200]: {resp.text[:200]}")
            return resp
        except Exception as e:
            logger.error(f"POST {url} error: {e}")
            return None

    async def get_scoring_periods(self) -> Optional[dict]:
        """Get scoring period info to find current period."""
        resp = await self._get(
            f"{self.BASE}/fxea/general/getLeagueInfo",
    ...
    async def get_player_stats_by_period(self) -> Optional[list]:
        """
        Use the Fantrax scoring endpoint which is more reliable.
        Returns flat list of player stat rows.
        """
        # Try the main stats page endpoint
        resp = await self._get(
            f"{self.BASE}/fxea/general/getStatsForPeriod",
            params={
        ...
        # Fallback: use the roster endpoint which we know works
        ...
        if resp2 and resp2.status_code == 200:
            try:
                ...
                if isinstance(data, dict):
                    logger.info(f"Roster full keys: {list(data.keys())}")
                    rosters = data.get("rosters", [])
                    logger.info(f"Number of roster entries: {len(rosters)}")
                    if rosters:
                        sample = rosters[0]
                        logger.info(f"First roster entry keys: {list(sample.keys()) if isinstance(sample, dict) else type(sample)}")
                        if isinstance(sample, dict):
                            items = sample.get("rosterItems", [])
                            if items:
                                logger.info(f"First player keys: {list(items[0].keys()) if isinstance(items[0], dict) else type(items[0])}")
            except Exception as e:
                logger.error(f"Roster inspect error: {e}")
        return None
...
async def run_fantrax_sync():
    ...
    try:
        ...
        # Inspect what Fantrax roster data actually looks like
        player_stats = await fantrax.get_player_stats_by_period()

        team_totals = {t: {"rbi":0,"strikeouts":0,"hits":0,"stolen_bases":0,"home_runs":0} for t in TEAMS}
        player_rows = []

        if player_stats:
            logger.info(f"Got {len(player_stats)} player stat rows")
            for p in player_stats:
                if not isinstance(p, dict):
                    continue
                fantasy_team = (p.get("teamName") or p.get("fantasyTeam") or
                                p.get("team") or p.get("leagueTeamName"))
                ...
                player_rows.append({
                    "player_id": pid or p.get("name", "unknown"),
            ...
            logger.warning("No player stats returned — writing zero totals")
        ...
        logger.info(f"✅ Sync complete — {len(player_rows)} players, 14 teams")
...
async def trigger_sync(request: Request):
    ...
    fantrax.logged_in = False  # Force re-login on next sync
