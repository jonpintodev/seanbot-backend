...
Uses fantraxapi library for reliable Fantrax data access
"""
...
# ── FANTRAX via fantraxapi library ────────────────────────────────────────────
COOKIES = {}
...
async def fantrax_request(methods: list) -> Optional[dict]:
    """
    Core Fantrax API call — matches how fantraxapi library works internally.
    Sends a POST with a 'methods' list to getLeagueInfo.
    """
    try:
        body = {"leagueId": FANTRAX_LEAGUE_ID, "methods": methods}
        async with httpx.AsyncClient(timeout=60, follow_redirects=True) as c:
            resp = await c.post(
                "https://www.fantrax.com/fxea/general/getLeagueInfo",
                json=body,
                headers=fantrax_headers()
            )
        for k, v in resp.cookies.items():
            COOKIES[k] = v
        logger.info(f"fantrax_request methods={methods}: {resp.status_code} | {resp.text[:500]}")
        if resp.status_code == 200:
            return resp.json()
    except Exception as e:
        logger.error(f"fantrax_request error: {e}")
    return None
...
async def run_fantrax_sync():
    ...
    try:
        ...
        # Step 1: Get rosters (we know this works)
        ...
        # Step 2: Try getLeagueInfo with methods list (fantraxapi approach)
        # Try different method names the library uses
        for method_name in ["scoring_periods", "scoreboard", "stats", "playerStats", "league_standings"]:
            result = await fantrax_request([method_name])
            if result and "error" not in result:
                logger.info(f"Method '{method_name}' worked! Keys: {list(result.keys())}")
                break

        # Step 3: Try getLeagueInfo directly (returns player pool info per docs)
        league_info = await fantrax_get(
            "/fxea/general/getLeagueInfo",
            {"leagueId": FANTRAX_LEAGUE_ID}
        )
        if isinstance(league_info, dict):
            logger.info(f"getLeagueInfo keys: {list(league_info.keys())}")
            # Log first 800 chars to see full structure
            logger.info(f"getLeagueInfo sample: {json.dumps(league_info)[:800]}")

        # Step 4: Try getting scoring periods via the library's approach
        scoring_data = await fantrax_request(["scoringPeriods"])
        if isinstance(scoring_data, dict):
            logger.info(f"scoringPeriods result keys: {list(scoring_data.keys())}")
            logger.info(f"scoringPeriods sample: {json.dumps(scoring_data)[:800]}")

        # Step 5: Try standings endpoint
        standings = await fantrax_get(
            "/fxea/general/getStandings",
            {"leagueId": FANTRAX_LEAGUE_ID}
        )
        if isinstance(standings, dict):
            logger.info(f"Standings keys: {list(standings.keys())}")
            logger.info(f"Standings sample: {json.dumps(standings)[:800]}")

        # Write zero totals for now (stats not yet pulled)
        ...
        if supabase:
            for team_name, stats in team_totals.items():
                supabase.table("team_stats").upsert({
                    ...
                    "rbi": stats["rbi"], "strikeouts": stats["strikeouts"],
                    "hits": stats["hits"], "stolen_bases": stats["stolen_bases"],
                    "home_runs": stats["home_runs"], "updated_at": today_str
        ...
        logger.info(f"Sync complete [{now_et}] — check logs for API structure")
